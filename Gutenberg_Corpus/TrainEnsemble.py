...
def train_ensemble(X_train,y_train):
    ...
    dtrees=[DTC(criterion='entropy',max_features='sqrt') for _ in range(n_trees)]
    trained_trees=[]
    for dt in dtrees:
        print(dt)
...
def combined_accuracy(preds,y_test):
    ...
    print("Preds shape: ",preds.shape,y_test.shape)
    #shape - NxT
    preds_df=pd.DataFrame(preds)
    preds_df['y_test']=y_test.tolist()
    print(preds_df.head())
...
if __name__ =='__main__':
   ...
   preds=np.zeros((X_test.shape[0],len(trained_trees)))

   for i,tree in enumerate(trained_trees):
       preds[:,i]=predict_tree(tree,X_test,y_test)
   
   preds_df,cacc=combined_accuracy(preds,y_test)  
   test_df=pd.DataFrame(dict(sentences=get_testsentences(X_test,new_vocab),y_test=y_test.tolist()))
