...
def validate_packet(packet):
    global start,end,destination,is_packet_validated,st,en,count_pkts,c
    ...
    if(count_pkts==2 and is_packet_valid(ttl,rtt)):
        ...
        count_pkts=0
        c.send(str(packet)) #send the packet on the connection.
...
def send_request(dst,qname,conn,port=53):
    global start,destination,hperiod,st,en, is_packet_validated,c
    import subprocess
    destination=dst
    c=conn
    st=time.time()
    ...
    en=time.time()
    #GIL in python helps here.
    is_packet_validated=False
    start=time.time()
    send(IP(dst=dst)/UDP(dport=port)/DNS(rd=5,qd=DNSQR(qname=qname)))
...
def hold_on_proxy(conn,qname="falun.com"):
    ...
    for attempt in range(3):
        ...
        send_request(eslrv2,qname=qname,conn=conn,port=5300)
...
def clientthread(conn):
    ...
    while(True):
        ...
        data = conn.recv(1024) #website address.
        reply = 'OK...' + data
        print(reply)
        expected_values()
        hold_on_proxy(conn,qname=data) #use conn to send packets.
        break
...
def create_socket(HOST='',PORT=53):
    ...
    while 1:
         ...
         #start new thread takes 1st argument as a
         #function name to be run, second is the tuple of arguments to the function.
